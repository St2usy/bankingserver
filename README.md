...
## 멱등성
연산을 여러번 적용해도 그 결과값이 같음을 의미한다. 즉 연산을 한번 적용하나, 여러번 적용하나 그 결과 값이 같다.

멱등성이 중요한 이유는 네트워크의 불안정성에서 기인한다. 네트워크의 패킷이 유실되기 때문에, 전송 자체가 실패하거나 응답이 바르지 않을 수 있다.

이때 멱등성이 보장된 비즈니스 로직이라면, 에러 상황에서 Clinet는 "Retry"해도, Server는 Clinet의 UUID를 확인 한 후, 단 한번만 연산을 처리한다.

멱등성의 작동 원리는, Client가 데이터 전송시 UUID를 헤더에 붙여서 전송한다. Server는 UUID를 확인한 후, 처리한 이력이 있는 UUID라면 연산을 처리하지 않는다.

이를 우리 비즈니스 로직에 적용해보자.
